...
import numpy as np
from hiperparameters import load_args_from_json
from AirsimEnv import AvoidDiscreteEnv, AvoidDiscreteOneDirEnv
...
class Agent(object):
    ...
    def state2input(self, state):
        if state[0].shape[1]>1: # omni-dir
            inputTensor0 = (torch.Tensor(state[0][:,0,:]).cpu(), 
                            torch.Tensor(state[0][:,1,:]).cpu(),
                            torch.Tensor(state[0][:,2,:]).cpu(),
                            torch.Tensor(state[0][:,3,:]).cpu())
        else:
            inputTensor0 = torch.Tensor(state[0][:,0,:]).cpu()
        inputTensor1 = torch.Tensor(state[1]).cpu()
...
class QLearning():
    ...
    def update(self,): 
        ...
        actions = torch.from_numpy(actions).type(torch.LongTensor).cpu()
        q_vals_pre_states =  self.agent.qnet_forward(pre_states).gather(1, actions.view(-1,1))
        loss = self.criterion(q_vals_pre_states.squeeze(-1), q_targets.cpu())
...
class TrainDQNAvoidance(TorchFlow.TorchFlow):
    def __init__(self, workingDir, args, prefix = "", suffix = "", plotterType = 'Visdom'):
        super(TrainDQNAvoidance, self).__init__(workingDir, prefix, suffix, disableStreamLogger = True, plotterType = plotterType)
        self.args = args
        self.prefix = prefix
        ...
        if self.args.omni_dir:
            self.env =  AvoidDiscreteEnv(period=self.period, framenum=self.frameNum, strictsafe=args.strict_safe) # DummyEnv() # 
        else:
            self.env =  AvoidDiscreteOneDirEnv(period=self.period, strictsafe=args.strict_safe)
    ...
    def initialize(self):
        super(TrainDQNAvoidance, self).initialize()
        # fill memory
        ...
        super(TrainDQNAvoidance, self).post_initialize()
    ...
    def train(self):
        super(TrainDQNAvoidance, self).train()
        self.countTrain += 1
    ...
    def alignCenterImgs(self, imglist, shapelist, imggap = 50):
        ...
        for imgshape,img in zip(shapelist,imglist):
            startx = int(imgh/2-imgshape[0]/2) # align middle 
            if len(imgshape)==2:
    ...
    def finalize(self):
        super(TrainDQNAvoidance, self).finalize()
...
args = load_args_from_json("config.json")
if args.use_int_plotter:
    plottertype = 'Int'
else:
    plottertype = 'Visdom'
    
    traindqn = TrainDQNAvoidance(args.working_dir, args, prefix = args.exp_prefix, plotterType = plottertype)
