...
class AvoidEnvBase(AirsimEnvBase):
    def __init__(self, period=0.1, camlist=[0], strictsafe=False):
        super(AvoidEnvBase, self).__init__(camlist=camlist) 
        self.client.reset()
        time.sleep(1)
...
class AvoidDiscreteOneDirEnv(AvoidEnvBase):
    def __init__(self, period=0.1,camlist=[0], strictsafe=False):
        super(AvoidDiscreteOneDirEnv, self).__init__(period=period, camlist=camlist, strictsafe=strictsafe)
...
class AvoidDiscreteEnv(AvoidDiscreteOneDirEnv):
...
class AvoidEnv(AvoidEnvBase):
    def __init__(self, period = 0.1):
        super(AvoidEnv, self).__init__()
        self.period = period
        # parameters for reward calculation
        self.lambda_forward = 1.0
        self.lambda_deviation = 0.1
...
if __name__ == '__main__':
    ...
    # test multiFrame
    env = AvoidDiscreteEnv(framenum = 10, strictsafe=True)
    env.reset()
    for k in range(200):
        state, reward, finish, _ = env.step(1)#np.random.randint(9))
